...
static void setKeyIv(CbdAes *pAes, jint type);
...
static void setKeyIv(CbdAes *aes, jint type) {
    if (type == 1) {
        aes->setAesKey((unsigned char *) KEY_TRACK, AES_BLOCK_SIZE);
        aes->setAesIv(const_cast<unsigned char *>(IV_TRACK), AES_BLOCK_SIZE);
    } else if (type == 2) {
        aes->setAesKey((unsigned char *) KEY_HTTP, AES_BLOCK_SIZE);
        aes->setAesIv(const_cast<unsigned char *>(IV_HTTP), AES_BLOCK_SIZE);
    } else {
        aes->setAesKey((unsigned char *) KEY_PASSWORD, AES_BLOCK_SIZE);
